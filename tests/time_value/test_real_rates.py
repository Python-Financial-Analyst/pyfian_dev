...
def test_fisher_real_rate_typical():
    assert pytest.approx(fisher_real_rate(0.05, 0.02), 0.0001) == 0.03

def test_fisher_exact_real_rate_typical():
    assert pytest.approx(fisher_exact_real_rate(0.05, 0.02), 0.000001) == 0.029412

def test_zero_nominal_rate():
    assert pytest.approx(fisher_real_rate(0.0, 0.02), 0.0001) == -0.02
    assert pytest.approx(fisher_exact_real_rate(0.0, 0.02), 0.000001) == -0.019608

def test_zero_inflation_rate():
...
def test_negative_inflation_rate():
    ...
    assert pytest.approx(fisher_exact_real_rate(0.05, -0.02), 0.000001) == 0.068627

def test_negative_nominal_rate():
    assert pytest.approx(fisher_real_rate(-0.01, 0.02), 0.0001) == -0.03
    assert pytest.approx(fisher_exact_real_rate(-0.01, 0.02), 0.000001) == -0.029412

def test_both_negative():
    assert pytest.approx(fisher_real_rate(-0.01, -0.02), 0.0001) == 0.01
    assert pytest.approx(fisher_exact_real_rate(-0.01, -0.02), 0.000001) == 0.010204

def test_equal_nominal_and_inflation():
