...
def test_mortgage_cash_flows_basic():
    ...
    assert len(df) == term_years * payment_frequency, "Number of payments should match term * frequency."
    ...
    assert abs(final_balance) < 1.0, f"Final remaining balance should be near zero, got {final_balance}"
...
def test_mortgage_cash_flows_zero_interest():
    ...
    assert all(df['Interest'] == 0), "With 0% interest, all interest payments should be zero."
    total_principal_paid = df['Principal'].sum()
    assert round(total_principal_paid, 2) == round(principal, 2), "Total principal payments should equal the original principal."
